class RegistrationsController < Devise::RegistrationsController
  ...
  # POST /resource
  def create
    resource=User.new(params[:user].except("perma_link"))
    unless params["user"]["perma_link"].blank?
     @parent_id=User.find_by_permalink(params["user"]["perma_link"])
     resource.parent_id=@parent_id.id
     end
    resource.errors
    resource.step="3"
    if resource.save
