...
describe BreakfastsController do
		describe "Breakfasts Controller for the askvee" do
		it "should get before_filtered for current_user" do
			controller.stub!(:current_user).and_return(true)
		end
					
			def mock_breakfast(stubs={})
    @mock_breakfast ||= mock_model(BreakFast, stubs).as_null_object
  end
  ...
  describe "GET index" do
	 ...
	 it "assigns all breakfast as @breakfast" do
	      BreakFast.stub(:all) { [mock_breakfast] }
	      get :index
	end
end
...
describe "responding to POST create" do
    ...
    describe "with valid params" do
      before(:each) do
        @description = "healthy Food",
				@is_available=true
        @breakfast_new = FactoryGirl.build(:break_fast, :description => @description, :is_available => @is_available)
        BreakFast.stub!(:new).and_return(@breakfast_new)
				@user=User.new(:parent_id=>"")
      end
			
			it "exposes a newly created breakfast as @breakfast " do
	
        post :create, :breakfast => { :name => ["dosai","porata"], :description => "Healthy and good Food", :is_available => true, :price => 123 ,:currency=>"Dollar"},:json_response=>{:success=>true},:format=>:json
	...
	describe "with invalid params" do
      it "assigns a newly created but unsaved user as @user " do
        @breakfast = FactoryGirl.build(:break_fast, :description => "", :price => "")
        BreakFast.stub!(:new).and_return(@breakfast)
